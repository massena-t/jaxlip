...
def assign_reparam_groups(model) -> None:
    ...
    groups: Dict[Tuple[int, Tuple[int, ...]], List[ReparametrizedModule]] = defaultdict(list)
    ...
    gid_map: Dict[Tuple[int, Tuple[int, ...]], int] = {}
    for gid, (key, mods) in enumerate(sorted(groups.items(), key=lambda kv: (kv[0][0], kv[0][1]))):
        gid_map[key] = gid
